    ...
    # with pdfplumber.open(file) as pdf:
        try:
            for page in pdf.pages:
                ...
                Approved =  text_with_boxes.split("61.A.O's name, sig & stamp")[0].split("DATE CODE LEVY")[1].strip()
                if Approved == '':
                    ...
                    Unit_Price = float(text_with_boxes.split("45.CUSTOM VALUE (PKR)")[1].split("37.ITEM NO 38.QUANTITY")[0].split("Assessed")[-1].split()[0].strip())
                    ...
                    GD_file_data_dict = {"Invoice_no":Invoice_no,"Name":Name,"Address":Address,"Place_of_Delivery":Place_of_Delivery,"Gross_WT":Gross_WT,"Net_WT":Net_WT,"Value":Value,"Country":Country,"No_of_packages":No_of_packages, "Importer_name_and_addr":Importer_name_and_addr, "Unit_price": Unit_Price, "No_of_units_2": No_of_units_2}
...
def Siddiqsons_Invoice(file):
    ...
    try:
        ...
        Invoice_ind = Invoice_df[Invoice_df.apply(lambda row: row.astype(str).str.contains('INVOICE #').any(), axis=1)]
        Invoice_no = Invoice_ind.iloc[0][1] if Invoice_ind.iloc[0][1] is not None else ''
    ...
    Place_of_Delivery_full = Invoice_df[Invoice_df.apply(lambda row: row.astype(str).str.contains('FROM:').any(), axis=1)]
    Place_of_Delivery_full = Place_of_Delivery_full.values.tolist()
    Place_of_Delivery_full = Place_of_Delivery_full[0]
    ind = 1
    for i in Place_of_Delivery_full:
        if "to" in i.lower():
            ind = Place_of_Delivery_full.index(i)
            break
    Place_of_Delivery_full = Place_of_Delivery_full[ind+1:]
    Place_of_Delivery = ""
    for j in Place_of_Delivery_full:
        if j != "":
            Place_of_Delivery = j
            break

    Place_of_Delivery = Place_of_Delivery.split("-")[0] if len(Place_of_Delivery.split("-")) > 1 else Place_of_Delivery.split(",")[0]
    Place_of_Delivery = Place_of_Delivery.strip()

    # Gross_WT = 0 if Invoice_df.iloc[64, 4] in [None, "None"] else Invoice_df.iloc[64, 4]
    matches = Invoice_df.applymap(lambda cell: 'GROSS WEIGHT' in str(cell))
    Gross_WT_row, Gross_WT_col = matches.where(matches).stack().index.tolist()[0] if len(matches.where(matches).stack().index.tolist()) < 2 else matches.where(matches).stack().index.tolist()[1]
    Gross_WT_col = Gross_WT_col.split(':')[1].strip() if len(Gross_WT_col.split(':')) > 1 else Gross_WT_col
    Gross_WT_col = int(Gross_WT_col)
    Gross_WT = Invoice_df.iloc[Gross_WT_row][Gross_WT_col +1] if Invoice_df.iloc[Gross_WT_row][Gross_WT_col +1] not in [None, ''] else Invoice_df.iloc[Gross_WT_row][Gross_WT_col +2]
    Gross_WT = 0 if Gross_WT =='' else Gross_WT
    Gross_WT = float(Gross_WT)/1000

    matches = Invoice_df.applymap(lambda cell: 'NET WEIGHT' in str(cell))
    Net_WT_row, Net_WT_col = matches.where(matches).stack().index.tolist()[0] if len(matches.where(matches).stack().index.tolist()) < 2 else matches.where(matches).stack().index.tolist()[1]
    Net_WT_col = Net_WT_col.split(':')[1].strip() if len(Net_WT_col.split(':')) > 1 else Net_WT_col
    Net_WT_col = int(Net_WT_col)
    Net_WT = Invoice_df.iloc[Net_WT_row][Net_WT_col +1] if Invoice_df.iloc[Net_WT_row][Net_WT_col +1] not in [None, ''] else Invoice_df.iloc[Net_WT_row][Net_WT_col +2]
    Net_WT = 0 if Net_WT =='' else Net_WT
    Net_WT = float(Net_WT)/1000

    try:
        matches = Invoice_df.applymap(lambda cell: 'NO. OF' in str(cell))
        packages_row, packages_col = matches.where(matches).stack().index.tolist()[0]
        packages_col = packages_col.split(':')[1].strip() if len(packages_col.split(':')) > 1 else packages_col
        package_ind = int(packages_row)
        packages_col = int(packages_col)
    except:
        matches = Invoice_df.applymap(lambda cell: 'Total  Cartons' in str(cell))
        packages_row, packages_col = matches.where(matches).stack().index.tolist()[0]
        packages_col = packages_col.split(':')[1].strip() if len(packages_col.split(':'))>1 else packages_col
        package_ind = int(packages_row)
        packages_col = int(packages_col)
    ...
    try:
        while True:
            num_of_Package = Invoice_df.iloc[i][packages_col]
    ...
    # Unit_Price = Invoice_df[Invoice_df.apply(lambda row: row.astype(str).str.contains('TOTAL GROSS WEIGHT').any(), axis=1)]
    # Unit_Price = Unit_Price.iloc[0][3] if Unit_Price.iloc[0][3] not in [None, ''] else Unit_Price.iloc[0][4]
    # Unit_Price = float(Gross_WT)/1000
    ...
    Country = Place_of_Delivery_full[0].strip()
    Country = Country.split("  ")[0]
    Country_1 = Country.split("-")[1] if len(Country.split("-")) > 1 else ""
    Country_2 = Country.split(",")[1] if len(Country.split(",")) > 1 else ""
    Country = Country_1.strip() if Country_1 != "" else Country_2.strip()
...
def Indigo_Invoice(file):
    ...
    Invoice_df = Invoice_df.fillna('')
    try:
        Invoice_ind = Invoice_df[Invoice_df.apply(lambda row: row.astype(str).str.contains('Invoice No').any(), axis=1)]
        Invoice_no = Invoice_ind.iloc[0][-2] if Invoice_ind.iloc[0][-2] is not None else ''
    except:
        Invoice_ind = Invoice_df[Invoice_df.apply(lambda row: row.astype(str).str.contains('Invoice no').any(), axis=1)]
        Invoice_no = Invoice_ind.iloc[0][-2] if Invoice_ind.iloc[0][-2] is not None else ''
    ...
    try:
        Name = Invoice_df[Invoice_df.apply(lambda row: row.astype(str).str.contains('FOR').any(), axis=1)]
        Name = Name.iloc[1][-4] if Name.iloc[1][-4] is not None else ''
        Name = Name.replace('FOR','').replace(':','')
    except:
        Name = Invoice_df[Invoice_df.apply(lambda row: row.astype(str).str.contains('For').any(), axis=1)]
        Name = Name.iloc[1][-4] if Name.iloc[1][-4] is not None else ''
        Name = Name.replace('For', '').replace(':', '')
...
def Akhtar_Invoice(file):
    ...
    try:
        Name = Invoice_df[Invoice_df.apply(lambda row: row.astype(str).str.contains('FOR').any(), axis=1)]
        Name = Name.iloc[0][1] if Name.iloc[0][1] is not None else ''
        Name = Name.replace('FOR','').replace(':','')
    except:
        Name = Invoice_df[Invoice_df.apply(lambda row: row.astype(str).str.contains('For').any(), axis=1)]
        Name = Name.iloc[0][-1] if Name.iloc[0][-1] is not None else ''
...
def Compare_files(GD_file_path, Invoice_path, Title):
    try:
        ...
        if len(GD_file_data) == 0:
            return {"matched":{}, "not_matched": {},"error": "GD already submitted"}
        ...
        Importer_name_and_addr = GD_file_data["Importer_name_and_addr"].lower().replace(',','').replace('.','') == Invoice_data["Importer_name_and_addr"].lower().replace(',','').replace('.','')
        ...
        Place_of_Delivery = GD_file_data["Place_of_Delivery"].lower().replace(',','').replace('.','') == Invoice_data["Place_of_Delivery"].lower().replace(',','').replace('.','')
...
# Invoice = 'C:\\Users\\admin\\Downloads\\CUSTOM INVOICE VESPO D.O # 658-24-25.xlsx'
# Invoice = 'C:\\Users\\admin\\Downloads\\DO SDM-0628-2024-2025 30.12.24.xlsx'
...
# Invoice = 'C:\\Users\\admin\\Downloads\\INV # 1285 DUMMY.xlsx'
...
# GD_file = 'C:\\Users\\admin\\Downloads\\rptViewer1 (1).pdf'
